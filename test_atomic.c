...
//atomic
	// ini
	temp = atomic_read(&st_info->delay);
	printk(KERN_INFO "st_info.delay:%d\n",temp);
	atomic_set(&st_info->delay, 50);
	temp = atomic_read(&st_info->delay);
	printk(KERN_INFO "st_info.delay:%d\n",temp);
	// add
	atomic_add(50, &st_info->delay);
	temp = atomic_read(&st_info->delay);
	printk(KERN_INFO "st_info.delay:%d\n",temp);

//bit
	set_bit(CMC_BIT_ALS, &st_info->enable);
	printk(KERN_INFO "st_info.enable:%lx\n",st_info->enable);
	set_bit(CMC_BIT_PS, &st_info->enable);
	printk(KERN_INFO "st_info.enable:%lx\n",st_info->enable);
	clear_bit(CMC_BIT_ALS, &st_info->enable);
	clear_bit(CMC_BIT_PS, &st_info->enable);
	printk(KERN_INFO "st_info.enable:%lx\n",st_info->enable);

	set_bit(CMC_BIT_ALS, &st_info->enable);
	temp = test_bit(CMC_BIT_ALS, &st_info->enable) ? (1) : (0) ;
